...
<body>
	...
	<article>
		...
		<p>所持金ランキングで現在${fn:escapeXml(pokerRanking[0].rank)}位です。一位との差額は${fn:escapeXml(pokerRanking[0].difference)}です</p>



		<p>・${fn:escapeXml(user.userName)}様のBJでの総獲得金額は${fn:escapeXml(bjSumList[0].sumCoin)}です。</p>

		<p>BJ総獲得金額ランキングで${fn:escapeXml(bjSumList[0].rank)}位です。一位との差額は${fn:escapeXml(bjSumList[0].difference)}です</p>



		<p>・${fn:escapeXml(user.userName)}様のポーカーでの総獲得金額は${fn:escapeXml(pokerSumList[0].sumCoin)}です。</p>

		<p>ポーカー総獲得金額ランキングで${fn:escapeXml(pokerSumList[0].rank)}位です。一位との差額は${fn:escapeXml(pokerSumList[0].difference)}です</p>



		<p>・${fn:escapeXml(user.userName)}様のポーカー1試合での最高獲得枚数は${fn:escapeXml(pokerOneTimeList[0].coin)}です。</p>

		<p>ポーカー1試合での最高獲得枚数ランキングで${fn:escapeXml(pokerOneTimeList[0].rank)}位です。一位との差額は${fn:escapeXml(pokerOneTimeList[0].difference)}です</p>
